...
export default function TypingTest() {
    ...
    if (isActive && timeLeft > 0 && !isFinished) {
      interval = setInterval(() => {
        ...
        if (userInput.length > 0) {
          ...
          const currentWmp =
            timeElapsed > 0 ? Math.round(words / timeElapsed) : 0;
  ...
  }, [isActive, timeLeft, isFinished, customTime]);

  const calculateStats = useCallback(() => {
    if (userInput.length === 0) return;

    const timeElapsed = startTime ? (Date.now() - startTime) / 60000 : 0.01;
    ...
    const rawWpm =
      timeElapsed > 0 ? Math.round(totalChars / 5 / timeElapsed) : 0;
  ...
  const toggleGhostMode = async () => {
    if (!ghostMode) {
      ...
      if (hasGhostData) {
        ...
        alert(
          "No previous run found! Complete a test first to enable ghost race mode."
        );
      }
    } else {
      setGhostMode(false);
    ...
    <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center">
            <div
              className={`text-2xl font-bold ${
                timeLeft <= 10 ? "text-error animate-pulse" : "text-primary"
              }`}
            >
              {timeLeft}s
            </div>
      ...
      >
        <textarea
          ...
          placeholder={
            isActive ? "Keep typing..." : "Click Start and begin typing here..."
          }
          value={userInput}
